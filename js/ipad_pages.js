...
if (window.innerWidth > 1078) {
  window.addEventListener("wheel", function (e) {
    ...
    if (e.deltaY > 0) {
      // 스크롤을 내릴 때
      if (
        pages[currentPage].getBoundingClientRect().bottom <= window.innerHeight
      ) {
        currentPage++;
...
const change = (e) => {
  ...
  filters.forEach((filter) => {
    //세개의 필터 어레이를 forEach로 돌림
    if (filter !== e.target) {
      // 현재 선택중인 필터만 제외하고 진행
      filter.childNodes.forEach((option) => {
        //두 필터를 forEach로 돌림 (옵션)
...
const createAll = (modelObj) => {
  ...
  const createcolor = () => {
    ...
    ipadColorBoxs.forEach((ipadColorBox, index) => {
      ...
      const cssColor = Object.keys(modelObj[index])
        .filter((key) => {
          return key.startsWith("cssColor");
        })
        .map((key) => {
          return modelObj[index][key];
        });
  ...
  // 가격 생성기
  const createPrice = () => {
    const ipad_prices = document.querySelectorAll(
      ".ipad_price_Wrap .ipad_price:nth-child(3)"
    );
    const ipad_Celprices = document.querySelectorAll(
      ".ipad_price_Wrap .ipad_price:nth-child(4)"
    );
    //wifi model price 넣기
    ipad_prices.forEach((ipad_price, index) => {
      const prices = Object.keys(modelObj[index])
        .filter((key) => {
          return key.startsWith("modelPrice");
        })
        .map((key) => {
          return modelObj[index][key];
        });
    ...
    ipad_Celprices.forEach((ipad_price, index) => {
      const prices = Object.keys(modelObj[index])
        .filter((key) => {
          return key.startsWith("modelCellularPrice");
        })
        .map((key) => {
          return modelObj[index][key];
        });
...
ipad_pages.forEach((ipad_page) => {
  ipad_page.addEventListener("mouseover", () => {
    subs.forEach((sub) => {
